...
Model::Model(ID3D11Device* dev, const char* const _modelName, float scale)
{
	mDevice = dev;
	pStdVerts = nullptr;
	pTriList = nullptr;
	...
	privLoadDataFromFile(_modelName, pStdVerts, numVerts, pTriList, numTris, scale);
	privLoadDataToGPU();
}
...
void Model::privLoadDataFromFile(const char* const _modelName, StandardVertex*& pVerts, int& nverts, TriangleByIndex*& ptlist, int& ntri, float scale)
{
	// Read from file
	FileHandle fh;
	FileError  ferror;
	...
	for (int i = 0; i < nverts; i++)
	{
		...
		v->set(tmp->x * scale, tmp->y * scale, tmp->z * scale, tmp->u, tmp->v, tmp->nx, tmp->ny, tmp->nz, tmp->r, tmp->g, tmp->b, (int)tmp->txt);
	}
...
void  Model::privLoadDataToGPU()
{
	...
	pMeshes = new MeshSeparator(pStdVerts, numVerts, pTriList, numTris);
...
void Model::RenderMesh(ID3D11DeviceContext* context, int meshnum)
{
	...
	pMeshes->GetMeshTriCountAndOffset(meshnum, tricount, trioffset);
