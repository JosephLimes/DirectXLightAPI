...
FlatPlane::FlatPlane(ID3D11Device* dev, float len, float urep, float vrep)
{
	...
	pVerts[0].set(-0.5f * len, 0, -0.5f * len, urep, vrep, up, Colors::Black);
	pVerts[1].set(-0.5f * len, 0, 0.5f * len, urep, 0, up, Colors::Blue);
	pVerts[2].set(0.5f * len, 0, 0.5f * len, 0, 0, up, Colors::Magenta);
	pVerts[3].set(0.5f * len, 0, -0.5f * len, 0, vrep, up, Colors::Red);
